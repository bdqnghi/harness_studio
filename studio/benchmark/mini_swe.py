...
class MiniSweBenchmark(Benchmark):
    ...
    def run(self, harness: Harness, task_ids, *, run_idx: int = 0) -> dict[str, float]:
        ...
        # ALWAYS honor force_build per invocation (see nexau.py: harbor's
        # non-force path uses the x86-only prebuilt image and crashes on arm64).
    ...
    def _trial_evidence(trial_dir: Path, tid: str):
        """Structured evidence: test verifier output as a signal + the mini-swe
        agent trajectory messages as the causal window."""
        reward = 0.0
        ...
        return evidence_from_trace(
            tid, reward, verifier_output=vtext,
            messages=MiniSweBenchmark._traj_messages(trial_dir / "agent"))
    ...
    @staticmethod
    def _traj_messages(agent_dir: Path) -> list[dict]:
        """The mini-swe-agent trajectory as a list of message dicts (or [])."""
